  ...
  const TRANSLATIONS = {
    en: {
      ...
      privacyPolicy: {
        ...
        transfersIntro: 'Some of the service providers we use to operate this site are based outside the European Union, which may result in the transfer of personal data — in particular your IP address — outside the EU. This is the case for:',
        transfersItem1: "Cloudflare, Inc., our hosting and content delivery provider, based in the United States (San Francisco), which serves the site's files and processes visitor connection data (IP addresses, request logs) as part of hosting;",
        transfersItem2: 'Google Fonts, from which this site loads typefaces (via fonts.googleapis.com and fonts.gstatic.com); font delivery can transmit your IP address to Google LLC, a company based in the United States;',
        transfersItem3: 'Supabase, our backend and database provider (accounts, orders), a company incorporated in the United States; depending on the infrastructure region configured for this project, some processing of your data may take place outside the EU;',
        transfersItem4: 'Stripe, our payment provider, a company incorporated in the United States operating global payment infrastructure; payment-related data may be processed outside the EU as part of transaction handling.',
        transfersSafeguards: 'Where such transfers occur, they are made subject to appropriate safeguards under the GDPR (such as standard contractual clauses, or reliance on the provider\'s own compliance framework), to the extent applicable to that provider. <span class="placeholder">[PLACEHOLDER — the exact safeguard(s) relied upon for each provider should be confirmed with a data protection professional and specified here before launch.]</span>',
        s7Heading: '8. Cookies',
    ...
    fr: {
      ...
      privacyPolicy: {
        ...
        transfersIntro: "Certains des prestataires de services que nous utilisons pour exploiter ce site sont basés en dehors de l'Union européenne, ce qui peut entraîner un transfert de données personnelles — notamment votre adresse IP — hors de l'UE. C'est le cas pour :",
        transfersItem1: "Cloudflare, Inc., notre hébergeur et prestataire de diffusion de contenu (CDN), basé aux États-Unis (San Francisco), qui héberge les fichiers du site et traite les données de connexion des visiteurs (adresses IP, journaux de requêtes) dans le cadre de l'hébergement ;",
        transfersItem2: "Google Fonts, à partir duquel ce site charge ses polices de caractères (via fonts.googleapis.com et fonts.gstatic.com) ; la livraison des polices peut transmettre votre adresse IP à Google LLC, une société basée aux États-Unis ;",
        transfersItem3: "Supabase, notre prestataire de backend et de base de données (comptes, commandes), une société de droit américain ; selon la région d'infrastructure configurée pour ce projet, certains traitements de vos données peuvent avoir lieu hors de l'UE ;",
        transfersItem4: "Stripe, notre prestataire de paiement, une société de droit américain opérant une infrastructure de paiement mondiale ; les données liées au paiement peuvent être traitées hors de l'UE dans le cadre du traitement des transactions.",
        transfersSafeguards: 'Lorsque de tels transferts ont lieu, ils sont encadrés par des garanties appropriées au regard du RGPD (telles que des clauses contractuelles types, ou le cadre de conformité propre au prestataire), dans la mesure applicable à ce prestataire. <span class="placeholder">[MARQUEUR — la ou les garanties précises retenues pour chaque prestataire doivent être confirmées auprès d\'un professionnel de la protection des données et précisées ici avant le lancement.]</span>',
        s7Heading: '8. Cookies',
